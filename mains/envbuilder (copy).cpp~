...
//ifstream fin("../descriptions/env_old.txt");
ofstream fout("../descriptions/env_new.txt");
...
void handleKeypress(unsigned char key, int x, int y) {
	switch (key) {
		...
		case '=': //rotate object about x-axis
			rot[0] += 1.0f;
			if(rot[0] >= 360)
				rot[0] -= 360;
			break;
		...
		case '0': //rotate object about y-axis
			rot[1] += 1.0f;
			if(rot[1] >= 360)
				rot[1] -= 360;
			break;

		case '9': //rotate object about y-axis
			rot[1] -= 1.0f;
			if(rot[1] < 0)
				rot[1] += 360;
			break;

		case '8': //rotate object about z-axis
			rot[2] += 1.0f;
			if(rot[2] >= 360)
				rot[2] -= 360;
			break;

		case '7': //rotate object about z-axis
			rot[2] -= 1.0f;
			if(rot[2] < 0)
				rot[2] += 360;
			break;
		...
		case 'm': //last obj
			obj_id--;
			if(obj_id == -1) obj_id = NUM_OF_OBJS - 1;
//			obj_id %= NUM_OF_OBJS;
			rot[0] = rot[1] = rot[2] = 0;
			pos[1] = 0;
			//objects_unit->get_dimentions((ObjName)obj_id, dims);
		...
		case '\r': //Enter Key: write object
			fout << obj_id << '\t'
				<< pos[0] << '\t' << pos[1] << '\t' << pos[2] << '\t'
				<< rot[0] << '\t' << rot[1] << '\t' << rot[2] << '\n';
			decor = new decorative((ObjName)obj_id, objects_unit);
			decor->setPos(pos[0], pos[1], pos[2]);
			decor->setRot(rot[0], rot[1], rot[2]);
...
// read old environment
/*    while(fin >> obj_id){
    	fin >> pos[0] >> pos[1] >> pos[2];
    	fin >> rot[0] >> rot[1] >> rot[2];
