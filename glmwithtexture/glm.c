...
/* glmMinMax: Calculates min and max of x,y,z
 * coordinates of a model.
 *
 * model   - initialized GLMmodel structure
 * minmax - array of 6 GLfloats
 * data will be stored in minmax in this order:
 * minx -> miny -> minz -> maxx -> maxy -> maxz
 */
GLvoid
glmMinMax(GLMmodel* model, GLfloat* minmax)
{
    GLuint i;
    GLfloat maxx, minx, maxy, miny, maxz, minz;
    ...
    /* set values in minmax to return */
    minmax[0] = minx;
    minmax[1] = miny;
    minmax[2] = minz;
    minmax[3] = maxx;
...
/* glmTranslate: Translates a model by a given amount.
 * 
 * model - properly initialized GLMmodel structure
 * translate - aray of 3 GLfloats
 */
GLvoid
glmTranslate(GLMmodel* model, GLfloat* translate)
{
    GLuint i;
